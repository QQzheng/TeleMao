...
public class ShareViewController: UIViewController, ShareViewDelegate, SAEFailedViewDelegate {
    ...
    private func buildAttachmentsAndPresentConversationPicker() {
        firstly { () -> Promise<[UnloadedItem]> in
            ...
            let alertTitle = NSLocalizedString("XXGJUSTHHANQS153",
                                               comment: "Shown when trying to share content to a Tmao user for the share extension. Followed by failure details.")
    ...
    struct LoadedItem {
        ...
        var customFileName: String? { isContactShare ? "Contact.vcf" : nil }
