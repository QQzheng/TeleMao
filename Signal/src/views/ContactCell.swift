    ...
    func updateSubtitle(subtitleType: SubtitleCellValue, contact: Contact) {
        ...
        case .phoneNumber:
            ...
            if let firstPhoneNumber = contact.userTextPhoneNumbers.first {
                ...
                self.subtitleLabel.text = NSLocalizedString("XXGJUSTHHANQC75", comment: "table cell subtitle when contact card has no known phone number")
            }
        case .email:
            self.textStackView.addArrangedSubview(self.subtitleLabel)
            ...
            if let firstEmail = contact.emails.first {
                ...
                self.subtitleLabel.text = NSLocalizedString("XXGJUSTHHANQC74", comment: "table cell subtitle when contact card has no email")
            }
        }
    }
