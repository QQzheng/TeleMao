...
- (void)showChangeAvatarUI
{
    ...
    ActionSheetAction *takePictureAction = [[ActionSheetAction alloc]
        initWithTitle:NSLocalizedString(@"XXGJUSTHHANQA60", @"media picker option to take photo or video")
...
- (void)takePicture
{
    ...
    [self.delegate.fromViewController ows_askForCameraPermissions:^(BOOL granted) {
        ...
        [self.delegate.fromViewController presentFullScreenViewController:picker animated:YES completion:nil];
    }];
}
        ...
        dismissViewControllerAnimated:YES
                           completion:^{
                               if (rawAvatar) {
                                   ...
                                   [self.delegate.fromViewController presentFullScreenViewController:vc
                                                                                  animated:YES
                                                                                completion:nil];
                               }
                           }];
