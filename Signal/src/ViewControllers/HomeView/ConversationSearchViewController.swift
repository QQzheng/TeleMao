    ...
    override func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        ...
        case .conversations:
            if searchResultSet.conversations.count > 0 {
                return NSLocalizedString("XXGJUSTHHANQS20", comment: "section header for search results that match existing conversations (either group or contact conversations)")
            } else {
                return nil
            }
        case .contacts:
            if searchResultSet.contacts.count > 0 {
                return NSLocalizedString("XXGJUSTHHANQS19", comment: "section header for search results that match a contact who doesn't have an existing conversation")
            } else {
                return nil
            }
        case .messages:
            if searchResultSet.messages.count > 0 {
                return NSLocalizedString("XXGJUSTHHANQA6", comment: "section header for search results that match a message in a conversation")
            } else {
                return nil
            }
        }
...
class EmptySearchResultCell: UITableViewCell {
    ...
    public func configure(searchText: String) {
        let format = NSLocalizedString("XXGJUSTHHANQH10", comment: "Format string when search returns no results. Embeds {{search term}}")
