...
//#import <StoreKit/StoreKit.h> //版本1.1 不走内购
...
- (void)loadView
{
    ...
    ReminderView *deregisteredView = [ReminderView
        nagWithText:TSAccountManager.sharedInstance.isPrimaryDevice
            ? NSLocalizedString(@"XXGJUSTHHANQD23", @"Label warning the user that they have been de-registered.")
            : NSLocalizedString(
                @"XXGJUSTHHANQU16", @"Label warning the user that they have been unlinked from their primary device.")
          tapAction:^{
    ...
    ReminderView *endOfLifeOSView =
        [ReminderView nagWithText:NSLocalizedString(@"XXGJUSTHHANQE28",
    ...
    ReminderView *outageView = [ReminderView
        nagWithText:NSLocalizedString(@"XXGJUSTHHANQO46", @"Label warning the user that the Tmao service may be down.")
          tapAction:nil];
    ...
    ReminderView *archiveReminderView =
        [ReminderView explanationWithText:NSLocalizedString(@"XXGJUSTHHANQI8",
    ...
    ReminderView *missingContactsPermissionView = [ReminderView
        nagWithText:NSLocalizedString(@"XXGJUSTHHANQI10",
...
{
    UIImageView *xxgjusthKongImgV = [UIImageView new];
    xxgjusthKongImgV.image = [UIImage imageNamed:@"kong"];
    xxgjusthKongImgV.layer.minificationFilter = kCAFilterTrilinear;
    xxgjusthKongImgV.layer.magnificationFilter = kCAFilterTrilinear;
    [xxgjusthKongImgV autoPinToAspectRatioWithSize:xxgjusthKongImgV.image.size];
    CGSize screenSize = UIScreen.mainScreen.bounds.size;
    CGFloat emptyInboxImageSize = MIN(screenSize.width, screenSize.height) * 0.65f;
    [xxgjusthKongImgV autoSetDimension:ALDimensionWidth toSize:emptyInboxImageSize];

    UILabel *xxgjusthKongLab = [UILabel new];
    xxgjusthKongLab.text = NSLocalizedString(@"XXGJUSTHHANQI9",
                                             @"无信息提醒");
    xxgjusthKongLab.font = UIFont.ows_dynamicTypeBodyClampedFont;
    xxgjusthKongLab.textColor = Theme.secondaryTextAndIconColor;
    xxgjusthKongLab.textAlignment = NSTextAlignmentCenter;
    xxgjusthKongLab.numberOfLines = 0;
    xxgjusthKongLab.lineBreakMode = NSLineBreakByWordWrapping;
    
    UIButton *xxgjusthKongBtn = [UIButton new];
    xxgjusthKongBtn.backgroundColor = UIColor.ows_signalBlueColor;
    NSString *tit = NSLocalizedString(@"XXGJUSTHHANQH4",
                                      @"按钮标题");
    [xxgjusthKongBtn setTitle:tit forState:UIControlStateNormal];
    [xxgjusthKongBtn setTitleColor:UIColor.whiteColor forState:UIControlStateNormal];
    xxgjusthKongBtn.titleLabel.font = UIFont.ows_dynamicTypeSubheadlineClampedFont;
    [xxgjusthKongBtn autoSetDimension:ALDimensionWidth toSize:emptyInboxImageSize];
    [xxgjusthKongBtn autoSetDimension:ALDimensionHeight toSize:34];
    xxgjusthKongBtn.layer.cornerRadius = 17;
    xxgjusthKongBtn.layer.masksToBounds = YES;
    [xxgjusthKongBtn addTarget:self action:@selector(showNewConversationView) forControlEvents:UIControlEventTouchUpInside];
    
    UIStackView *xxgjusthKongView = [[UIStackView alloc] initWithArrangedSubviews:@[
                                                                                   xxgjusthKongImgV,
                                                                                   xxgjusthKongLab,
                                                                                   xxgjusthKongBtn,
                                                                                   ]];
    xxgjusthKongView.axis = UILayoutConstraintAxisVertical;
    xxgjusthKongView.alignment = UIStackViewAlignmentCenter;
    xxgjusthKongView.spacing = 12;
    xxgjusthKongView.layoutMargins = UIEdgeInsetsMake(50, 50, 50, 50);
    xxgjusthKongView.layoutMarginsRelativeArrangement = YES;
    return xxgjusthKongView;
}
...
- (void)viewDidLoad
{
    ...
    switch (self.conversationListMode) {
        case ConversationListMode_Inbox:
            ...
            self.title
                = NSLocalizedString(@"XXGJUSTHHANQA36", @"Title for the conversation list's default mode.");
            break;
        case ConversationListMode_Archive:
            self.title
                = NSLocalizedString(@"XXGJUSTHHANQA37", @"Title for the conversation list's 'archive' mode.");
            break;
    }
    ...
    self.searchBar.placeholder = NSLocalizedString(@"XXGJUSTHHANQH3",
...
- (void)updateBarButtonItems
{
    ...
    UIBarButtonItem *compose = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"home_bianji"]
                                                                style:UIBarButtonItemStylePlain
                                                               target:self
    ...
    compose.accessibilityIdentifier = ACCESSIBILITY_IDENTIFIER_WITH_NAME(self, @"home_bianji");
...
- (void)showNewConversationView
{
    ...
    [self.contactsManager requestSystemContactsOnceWithCompletion:^(NSError *_Nullable error) {
        ...
        [self.navigationController presentFullScreenViewController:modal animated:YES completion:nil];
    }];
}
...
- (UITableViewCell *)cellForArchivedConversationsRow:(UITableView *)tableView
{
    ...
    label.text = NSLocalizedString(@"XXGJUSTHHANQH1", @"Label for 'archived conversations' button.");
        ...
        case ConversationListViewControllerSectionConversations: {
            UITableViewRowAction *deleteAction =
                [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDefault
                                                   title:NSLocalizedString(@"XXGJUSTHHANQT20", nil)
            ...
            if (self.conversationListMode == ConversationListMode_Inbox) {
                archiveAction = [UITableViewRowAction
                    rowActionWithStyle:UITableViewRowActionStyleNormal
                                 title:NSLocalizedString(@"XXGJUSTHHANQA37",
                ...
                archiveAction = [UITableViewRowAction
                    rowActionWithStyle:UITableViewRowActionStyleNormal
                                 title:NSLocalizedString(@"XXGJUSTHHANQU1",
...
- (void)tableViewCellTappedDelete:(NSIndexPath *)indexPath
{
    ...
    ActionSheetController *alert = [[ActionSheetController alloc]
        initWithTitle:NSLocalizedString(@"XXGJUSTHHANQC85",
                          @"Title for the 'conversation delete confirmation' alert.")
              message:NSLocalizedString(@"XXGJUSTHHANQC84",
                          @"Message for the 'conversation delete confirmation' alert.")];
    [alert addAction:[[ActionSheetAction alloc] initWithTitle:NSLocalizedString(@"XXGJUSTHHANQT20", nil)
