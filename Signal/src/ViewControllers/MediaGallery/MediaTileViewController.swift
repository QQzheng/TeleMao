...
public class MediaTileViewController: UICollectionViewController, MediaGalleryDelegate, UICollectionViewDelegateFlowLayout {
    ...
    override public func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        guard galleryDates.count > 0 else {
            ...
            let title = NSLocalizedString("XXGJUSTHHANQG1", comment: "Label indicating media gallery is empty")
        ...
        if (kind == UICollectionView.elementKindSectionHeader) {
            ...
            case kLoadOlderSectionIdx:
                ...
                let title = NSLocalizedString("XXGJUSTHHANQG3", comment: "Label indicating loading is in progress")
                sectionHeader.configure(title: title)
                return sectionHeader
            case loadNewerSectionIdx:
                ...
                let title = NSLocalizedString("XXGJUSTHHANQG2", comment: "Label indicating loading is in progress")
                sectionHeader.configure(title: title)
                return sectionHeader
            default:
    ...
    func updateSelectButton() {
        if isInBatchSelectMode {
            ...
            self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: NSLocalizedString("XXGJUSTHHANQB46", comment: "Button text to enable batch selection mode"),
                                                                     style: .plain,
                                                                     target: self,
    ...
    func didPressDelete(_ sender: Any) {
        ...
        let confirmationTitle: String = {
            if indexPaths.count == 1 {
                return NSLocalizedString("XXGJUSTHHANQM4", comment: "Confirmation button text to delete selected media message from the gallery")
            } else {
                let format = NSLocalizedString("XXGJUSTHHANQM3", comment: "Confirmation button text to delete selected media from the gallery, embeds {{number of messages}}")
