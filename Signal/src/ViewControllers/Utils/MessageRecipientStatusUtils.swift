...
public class MessageRecipientStatusUtils: NSObject {
    ...
    class func recipientStatusAndStatusMessage(outgoingMessage: TSOutgoingMessage,
        ...
        case .failed:
            let shortStatusMessage = NSLocalizedString("XXGJUSTHHANQM42", comment: "status message for failed messages")
            let longStatusMessage = NSLocalizedString("XXGJUSTHHANQM41", comment: "status message for failed messages")
        ...
        case .sending:
            if outgoingMessage.hasAttachments() {
                ...
                let statusMessage = NSLocalizedString("XXGJUSTHHANQM48",
                ...
                let statusMessage = NSLocalizedString("XXGJUSTHHANQM46",
        ...
        case .sent:
            if let readTimestamp = recipientState.readTimestamp {
                ...
                let longStatusMessage = NSLocalizedString("XXGJUSTHHANQM43", comment: "status message for read messages") + " " + timestampString
            ...
            if let deliveryTimestamp = recipientState.deliveryTimestamp {
                ...
                let shortStatusMessage = timestampString
                let longStatusMessage = NSLocalizedString("XXGJUSTHHANQM39",
            ...
            let statusMessage =
                NSLocalizedString("XXGJUSTHHANQM47",
        ...
        case .skipped:
            let statusMessage = NSLocalizedString("XXGJUSTHHANQM44",
                                                  comment: "message status if message delivery to a recipient is skipped. We skip delivering group messages to users who have left the group or unregistered their Tmao account.")
    ...
    internal class func receiptStatusAndMessage(outgoingMessage: TSOutgoingMessage) -> (status: MessageReceiptStatus, message: String) {
        ...
        case .failed:
            ...
            return (.failed, NSLocalizedString("XXGJUSTHHANQM41", comment: "status message for failed messages"))
        case .sending:
            if outgoingMessage.hasAttachments() {
                return (.uploading, NSLocalizedString("XXGJUSTHHANQM48",
                                         comment: "status message while attachment is uploading"))
            } else {
                return (.sending, NSLocalizedString("XXGJUSTHHANQM46",
                                         comment: "message status while message is sending."))
            }
        case .sent:
            if outgoingMessage.readRecipientAddresses().count > 0 {
                return (.read, NSLocalizedString("XXGJUSTHHANQM43", comment: "status message for read messages"))
            }
            if outgoingMessage.wasDeliveredToAnyRecipient {
                return (.delivered, NSLocalizedString("XXGJUSTHHANQM39",
                                         comment: "message status for message delivered to their recipient."))
            }
            return (.sent, NSLocalizedString("XXGJUSTHHANQM47",
                                     comment: "status message for sent messages"))
        default:
