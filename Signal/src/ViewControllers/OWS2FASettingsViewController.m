...
- (void)viewDidLoad
{
    ...
    self.title = NSLocalizedString(@"XXGJUSTHHANQE25", @"Title for the 'enable two factor auth PIN' views.");
    ...
    if (self.mode != OWS2FASettingsMode_Status) {
        UIBarButtonItem *nextButton = [[UIBarButtonItem alloc]
                      initWithTitle:NSLocalizedString(@"XXGJUSTHHANQB44",
...
- (void)createStatusContents
{
    ...
    NSString *instructions = ([OWS2FAManager.sharedManager is2FAEnabled]
            ? NSLocalizedString(@"XXGJUSTHHANQE24",
                  @"Indicates that user has 'two factor auth pin' enabled.")
            : NSLocalizedString(@"XXGJUSTHHANQE23",
...
{
    [self createEnterPINContentsWithInstructions:NSLocalizedString(@"XXGJUSTHHANQE22",
...
{
    [self
        createEnterPINContentsWithInstructions:NSLocalizedString(@"XXGJUSTHHANQE13",
            ...
            if ([OWS2FAManager.sharedManager is2FAEnabled]) {
                [section
                    addItem:[OWSTableItem disclosureItemWithText:
                                              NSLocalizedString(@"XXGJUSTHHANQE16",
                ...
                [section addItem:[OWSTableItem
                                      disclosureItemWithText:
                                          NSLocalizedString(@"XXGJUSTHHANQE17",
                                              @"Label for the 'enable two-factor auth' item in the settings view")
    ...
    switch (self.mode) {
        ...
        case OWS2FASettingsMode_ConfirmPIN: {
            ...
            if ([self.pinTextfield.text isEqualToString:self.candidatePin]) {
                ...
                self.errorLabel.text = NSLocalizedString(@"XXGJUSTHHANQE19",
...
- (BOOL)hasValidPin
{
    if (self.pinTextfield.text.length < kMin2FAPinLength) {
        ...
        self.errorLabel.text = NSLocalizedString(
            @"XXGJUSTHHANQE21", @"Error indicating that the entered 'two-factor auth PIN' is too short.");
        return NO;
    }
    ...
    if (self.pinTextfield.text.length > kMax2FAv1PinLength) {
        self.errorLabel.hidden = NO;
        self.errorLabel.text = NSLocalizedString(
            @"XXGJUSTHHANQE20", @"Error indicating that the entered 'two-factor auth PIN' is too long.");
        return NO;
    }
                          ...
                          failure:^(NSError *error) {
                              [modalActivityIndicator dismissWithCompletion:^{
                                  ...
                                  [OWSActionSheets showErrorAlertWithMessage:
                                                       NSLocalizedString(@"XXGJUSTHHANQE14",
                      ...
                      [OWS2FAManager.sharedManager requestEnable2FAWithPin:self.candidatePin
                          ...
                          failure:^(NSError *error) {
                              [modalActivityIndicator dismissWithCompletion:^{
                                  ...
                                  [OWSActionSheets showErrorAlertWithMessage:
                                                       NSLocalizedString(@"XXGJUSTHHANQE15",
