...
+ (void)presentFromViewController:(UIViewController *)viewController address:(SignalServiceAddress *)address
{
    ...
    if (!recipientIdentity) {
        [OWSActionSheets showActionSheetWithTitle:NSLocalizedString(@"XXGJUSTHHANQA19",
                                                      @"Title for alert explaining that a user cannot be verified.")
                                          message:NSLocalizedString(@"XXGJUSTHHANQC27",
    ...
    [viewController presentFullScreenViewController:navigationController animated:YES completion:nil];
}
...
- (void)loadView
{
    ...
    self.title = NSLocalizedString(@"XXGJUSTHHANQP80", @"Navbar title");
...
- (void)createViews
{
    ...
    learnMoreLabel.attributedText = [[NSAttributedString alloc]
        initWithString:NSLocalizedString(@"XXGJUSTHHANQP69",
    ...
    // Instructions
    NSString *instructionsFormat = NSLocalizedString(@"XXGJUSTHHANQP79",
    ...
    scanLabel.text = NSLocalizedString(@"XXGJUSTHHANQP70", @"Button that shows the 'scan with camera' view.");
...
- (void)updateVerificationStateLabel
{
    ...
    if (isVerified) {
        ...
        [labelText
            appendAttributedString:
                [[NSAttributedString alloc]
                    initWithString:[NSString stringWithFormat:NSLocalizedString(@"XXGJUSTHHANQP68",
        ...
        self.verifyUnverifyButtonLabel.text = NSLocalizedString(
            @"XXGJUSTHHANQP71", @"Button that lets user mark another user's identity as unverified.");
    } else {
        self.verificationStateLabel.text = [NSString
            stringWithFormat:NSLocalizedString(@"XXGJUSTHHANQP66",
        ...
        [buttonText appendAttributedString:
                        [[NSAttributedString alloc]
                            initWithString:NSLocalizedString(@"XXGJUSTHHANQP81",
...
- (void)showSharingActivityWithCompletion:(nullable void (^)(void))completionHandler
{
    ...
    NSString *shareFormat = NSLocalizedString(
        @"XXGJUSTHHANQS4", @"Snippet to share {{safety number}} with a friend. sent e.g. via SMS");
    ...
    [self presentFullScreenViewController:activityController animated:YES completion:nil];
}
