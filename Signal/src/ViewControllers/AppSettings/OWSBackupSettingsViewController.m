...
- (void)viewDidLoad
{
    ...
    self.title = NSLocalizedString(@"XXGJUSTHHANQS65", @"Label for the backup view in app settings.");
...
- (void)updateTableContents
{
    ...
    if (self.iCloudError) {
        ...
        iCloudSection.headerTitle = NSLocalizedString(
            @"XXGJUSTHHANQS69", @"Label for iCloud status row in the in the backup settings view.");
        [iCloudSection
    ...
    enableSection.headerTitle = NSLocalizedString(@"XXGJUSTHHANQS65", @"Label for the backup view in app settings.");
    [enableSection
        addItem:[OWSTableItem switchItemWithText:
                                  NSLocalizedString(@"XXGJUSTHHANQS68",
    ...
    if (isBackupEnabled) {
        ...
        [progressSection
            addItem:[OWSTableItem
                        labelItemWithText:NSLocalizedString(@"XXGJUSTHHANQB19",
        ...
        if (OWSBackup.sharedManager.backupExportState == OWSBackupState_InProgress) {
            if (OWSBackup.sharedManager.backupExportDescription) {
                [progressSection
                    addItem:[OWSTableItem
                                labelItemWithText:NSLocalizedString(@"XXGJUSTHHANQS74",
                ...
                if (OWSBackup.sharedManager.backupExportProgress) {
                    ...
                    [progressSection
                        addItem:[OWSTableItem
                                    labelItemWithText:NSLocalizedString(@"XXGJUSTHHANQB18",
                                                          @"Label for phase row in the in the backup settings view.")
        ...
        switch (OWSBackup.sharedManager.backupExportState) {
            ...
            case OWSBackupState_Succeeded:
                [progressSection
                    addItem:[OWSTableItem disclosureItemWithText:
                                              NSLocalizedString(@"XXGJUSTHHANQS66",
            ...
            case OWSBackupState_InProgress:
                [progressSection
                    addItem:[OWSTableItem disclosureItemWithText:
                                              NSLocalizedString(@"XXGJUSTHHANQS67",
