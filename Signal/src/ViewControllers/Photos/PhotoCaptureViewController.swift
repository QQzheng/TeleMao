...
extension PhotoCaptureError: LocalizedError {
    var localizedDescription: String {
        switch self {
        case .initializationFailed:
            return NSLocalizedString("XXGJUSTHHANQP26", comment: "alert title")
        case .captureFailed:
            return NSLocalizedString("XXGJUSTHHANQP25", comment: "alert title")
        case .assertionError:
            return NSLocalizedString("XXGJUSTHHANQP24", comment: "alert title, generic error preventing user from capturing a photo")
        }
    }
}
