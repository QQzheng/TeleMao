...
class SendMediaNavigationController: OWSNavigationController {
    ...
    private func didRequestExit(dontAbandonText: String) {
        if attachmentDraftCollection.count == 0 {
            ...
            let confirmAbandonText = NSLocalizedString("XXGJUSTHHANQS44", comment: "alert action, confirming the user wants to exit the media flow and abandon any photos they've taken")
...
extension SendMediaNavigationController: UINavigationControllerDelegate {
    ...
    func showTooManySelectedToast() {
        ...
        let toastFormat = NSLocalizedString("XXGJUSTHHANQI1",
...
extension SendMediaNavigationController: PhotoCaptureViewControllerDelegate {
    ...
    func photoCaptureViewControllerDidCancel(_ photoCaptureViewController: PhotoCaptureViewController) {
        let dontAbandonText = NSLocalizedString("XXGJUSTHHANQS45", comment: "alert action when the user decides not to cancel the media flow after all.")
...
extension SendMediaNavigationController: ImagePickerGridControllerDelegate {
    ...
    func imagePickerDidCancel(_ imagePicker: ImagePickerGridController) {
        let dontAbandonText = NSLocalizedString("XXGJUSTHHANQS46", comment: "alert action when the user decides not to cancel the media flow after all.")
        didRequestExit(dontAbandonText: dontAbandonText)
    }
    ...
    func showApprovalAfterProcessingAnyMediaLibrarySelections() {
        let backgroundBlock: (ModalActivityIndicatorViewController) -> Void = { modal in
            when(fulfilled: self.attachmentDraftCollection.attachmentApprovalItemPromises).map { attachmentApprovalItems in
                ...
                modal.dismiss {
                    OWSActionSheets.showActionSheet(title: NSLocalizedString("XXGJUSTHHANQI2", comment: "alert title"))
                }
            }.retainUntilComplete()
        }
