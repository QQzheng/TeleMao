...
class ColorPickerView: UIView, ColorViewDelegate {
    ...
    private func buildHeaderView() -> UIView {
        ...
        titleLabel.text = NSLocalizedString("XXGJUSTHHANQC39", comment: "Modal Sheet title when picking a conversation color.")
    ...
    private var outgoingViewItem: MockConversationViewItem {
        ...
        let outgoingText = NSLocalizedString("XXGJUSTHHANQC37", comment: "The first of two messages demonstrating the chosen conversation color, by rendering this message in an outgoing message bubble.")
    ...
    private var incomingViewItem: MockConversationViewItem {
        let thread = MockThread(contactAddress: SignalServiceAddress(phoneNumber: "+fake-id"))
        let incomingText = NSLocalizedString("XXGJUSTHHANQC38", comment: "The second of two messages demonstrating the chosen conversation color, by rendering this message in an incoming message bubble.")
