...
- (UILabel *)configureQuotedTextLabel
{
    ...
    if (self.displayableQuotedText.displayText.length > 0) {
        ...
        text = NSLocalizedString(
                                 @"XXGJUSTHHANQA2", @"Indicates this message is a quoted reply to an attachment of unknown type.");
        textColor = self.fileTypeTextColor;
        font = self.fileTypeFont;
    }
...
- (UILabel *)configureQuoteContentSourceLabel
{
    ...
    self.quoteContentSourceLabel.text = NSLocalizedString(@"XXGJUSTHHANQQ5",
...
- (nullable NSString *)fileTypeForSnippet
{
    ...
    if ([MIMETypeUtil isAudio:contentType]) {
        return NSLocalizedString(
            @"XXGJUSTHHANQA3", @"Indicates this message is a quoted reply to an audio file.");
    } else if ([MIMETypeUtil isVideo:contentType]) {
        return NSLocalizedString(
            @"XXGJUSTHHANQQ11", @"Indicates this message is a quoted reply to a video file.");
    } else if ([MIMETypeUtil isImage:contentType]) {
        return NSLocalizedString(
            @"XXGJUSTHHANQQ10", @"Indicates this message is a quoted reply to an image file.");
    } else if ([MIMETypeUtil isAnimated:contentType]) {
        return NSLocalizedString(
            @"XXGJUSTHHANQQ9F", @"Indicates this message is a quoted reply to animated GIF file.");
    }
    return nil;
}
    ...
    if (self.quotedMessage.authorAddress.isLocalAddress) {
        ...
        if (self.isOutgoing) {
            quotedAuthorText = NSLocalizedString(
                @"XXGJUSTHHANQQ4", @"message header label when quoting yourself");
        } else {
            quotedAuthorText = NSLocalizedString(
                @"XXGJUSTHHANQQ3", @"message header label when someone else is quoting you");
        }
    } else {
        ...
        quotedAuthorText = [NSString
            stringWithFormat:
                NSLocalizedString(@"XXGJUSTHHANQQ2",
