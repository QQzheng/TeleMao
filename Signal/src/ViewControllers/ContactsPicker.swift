...
public class ContactsPicker: OWSViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    override open func viewDidLoad() {
        ...
        searchBar.placeholder = NSLocalizedString("XXGJUSTHHANQH3", comment: "Search")
    ...
    private func getContacts(onError errorHandler: @escaping (_ error: Error) -> Void) {
        switch CNContactStore.authorizationStatus(for: CNEntityType.contacts) {
            case CNAuthorizationStatus.denied, CNAuthorizationStatus.restricted:
                let title = NSLocalizedString("XXGJUSTHHANQI20", comment: "Alert title when contacts disabled while trying to invite contacts to Tmao")
                let body = NSLocalizedString("XXGJUSTHHANQI19", comment: "Alert body when contacts disabled while trying to invite contacts to Tmao")
