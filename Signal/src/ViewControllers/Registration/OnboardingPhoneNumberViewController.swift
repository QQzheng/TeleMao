...
public class OnboardingPhoneNumberViewController: OnboardingBaseViewController {
    ...
    private var viewHei:CGFloat = 0.0
    ...
    override public func loadView() {
       ...
       bgview.autoSetDimension(.height, toSize: gao)
       bgview.autoPin(toTopLayoutGuideOf: self, withInset: -statusH)
       primaryView.addSubview(bgview)
        
        let titleLabel = self.titleLabel(text: NSLocalizedString("XXGJUSTHHANQO26", comment: "注册页面标题"))
        titleLabel.accessibilityIdentifier = "onboarding.phoneNumber." + "titleLabel"
        titleLabel.autoSetDimension(.height, toSize: 33)
        ...
        validationWarningLabel.text = NSLocalizedString("XXGJUSTHHANQO27",
...
//        let bottomSpacer = UIView.vStretchingSpacer()
        ...
        stackView.autoPinWidthToSuperview(withMargin: 32)
        stackView.autoPin(toTopLayoutGuideOf: self, withInset: gao)

        let font = UIFont.ows_dynamicTypeBodyClamped.ows_semibold()
        let buttonHeight = OWSFlatButton.heightForFont(font)
        let hh : CGFloat = gao + validationWarningLabel.font.lineHeight + buttonHeight + 142 + statusH
        addKeyboardNoti(height: hh)

        validationWarningLabel.autoPinEdge(.leading, to: .leading, of: phoneNumberTextField)
    }
    
    private func addKeyboardNoti(height: CGFloat){
        viewHei = height
        NotificationCenter.default.addObserver(self, selector: #selector(keyboardWillShow(notice:)), name: UIResponder.keyboardWillShowNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(keyboardWillHide(notice:)), name: UIResponder.keyboardWillHideNotification, object: nil)
    }
    
    @objc private func keyboardWillShow(notice: NSNotification){
        let info: Dictionary = notice.userInfo!
        ...
        let maxY = viewHei
        let cha = boundHeight - keyBoderHeight
        if cha < maxY {
            let gao = cha - maxY - 10
            weak var weakSelf = self
            UIView.animate(withDuration: duration, animations: {
               weakSelf!.view.frame = CGRect(x: weakSelf!.view.frame.origin.x, y: weakSelf!.view.frame.origin.y+gao, width: weakSelf!.view.frame.size.width, height: weakSelf!.view.frame.size.height)
            })
        }
    }
    @objc func keyboardWillHide(notice:NSNotification){
        let info: Dictionary = notice.userInfo!
        let duration = (info[UIResponder.keyboardAnimationDurationUserInfoKey] as! NSNumber).doubleValue
        if view.frame.origin.y < 0  {
            weak var weakSelf = self
            UIView.animate(withDuration: duration, animations: {
                weakSelf!.view.frame = CGRect(x: weakSelf!.view.frame.origin.x, y: 0, width: weakSelf!.view.frame.size.width, height: weakSelf!.view.frame.size.height)
            })
        }
