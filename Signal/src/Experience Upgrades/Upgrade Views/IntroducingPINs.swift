...
class IntroducingPinsMegaphone: MegaphoneView {
    init(experienceUpgrade: ExperienceUpgrade, fromViewController: UIViewController) {
        ...
        titleText = hasPinAlready
            ? NSLocalizedString("XXGJUSTHHANQP59", comment: "Title for PIN megaphone when user already has a PIN")
            : NSLocalizedString("XXGJUSTHHANQP63", comment: "Title for PIN megaphone when user doesn't have a PIN")
        bodyText = hasPinAlready
            ? NSLocalizedString("XXGJUSTHHANQP57", comment: "Body for PIN megaphone when user already has a PIN")
            : NSLocalizedString("XXGJUSTHHANQP61", comment: "Body for PIN megaphone when user doesn't have a PIN")
        imageName = "PIN_megaphone"

        let primaryButtonTitle = hasPinAlready
            ? NSLocalizedString("XXGJUSTHHANQP56", comment: "Action text for PIN megaphone when user already has a PIN")
            : NSLocalizedString("XXGJUSTHHANQP60", comment: "Action text for PIN megaphone when user doesn't have a PIN")
        ...
        let primaryButton = MegaphoneView.Button(title: primaryButtonTitle) { [weak self] in
            let vc = PinSetupViewController {
                ...
                fromViewController.navigationController?.popToViewController(fromViewController, animated: true) {
                    ...
                    self?.presentToast(
                        text: NSLocalizedString("XXGJUSTHHANQP64", comment: "Toast indicating that a PIN has been created."),
...
class IntroducingPinsSplash: SplashViewController {
    ...
    override func loadView() {
        ...
        if hasPinAlready {
            title = NSLocalizedString("XXGJUSTHHANQU27", comment: "Header for PINs migration splash screen")
            body = NSLocalizedString("XXGJUSTHHANQU26", comment: "Body text for PINs migration splash screen")
        } else {
            title = NSLocalizedString("XXGJUSTHHANQU29", comment: "Header for PINs splash screen")
            body = NSLocalizedString("XXGJUSTHHANQU28", comment: "Body text for PINs splash screen")
        }
    ...
    func primaryButtonTitle() -> String {
        return NSLocalizedString("XXGJUSTHHANQU24",
