...
extension GiphyError: LocalizedError {
    public var errorDescription: String? {
        switch self {
        case .assertionError:
            return NSLocalizedString("XXGJUSTHHANQG5", comment: "Generic error displayed when picking a GIF")
        case .fetchFailure:
            return NSLocalizedString("XXGJUSTHHANQG4", comment: "Error displayed when there is a failure fetching a GIF from the remote service.")
        }
    }
}
