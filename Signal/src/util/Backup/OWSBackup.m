...
NSString *NSStringForBackupExportState(OWSBackupState state)
{
    switch (state) {
        case OWSBackupState_Idle:
            return NSLocalizedString(@"XXGJUSTHHANQS76", @"Indicates that app is not backing up.");
        case OWSBackupState_InProgress:
            return NSLocalizedString(@"XXGJUSTHHANQS77", @"Indicates that app is backing up.");
        case OWSBackupState_Failed:
            return NSLocalizedString(@"XXGJUSTHHANQS75", @"Indicates that the last backup failed.");
        case OWSBackupState_Succeeded:
            return NSLocalizedString(@"XXGJUSTHHANQS78", @"Indicates that the last backup succeeded.");
    }
}
...
NSString *NSStringForBackupImportState(OWSBackupState state)
{
    switch (state) {
        case OWSBackupState_Idle:
            return NSLocalizedString(@"XXGJUSTHHANQS71", @"Indicates that app is not restoring up.");
        case OWSBackupState_InProgress:
            return NSLocalizedString(
                @"XXGJUSTHHANQS72", @"Indicates that app is restoring up.");
        case OWSBackupState_Failed:
            return NSLocalizedString(
                @"XXGJUSTHHANQS70", @"Indicates that the last backup restore failed.");
        case OWSBackupState_Succeeded:
            return NSLocalizedString(
                @"XXGJUSTHHANQS73", @"Indicates that the last backup restore succeeded.");
    }
}
...
- (AnyPromise *)ensureCloudKitAccess
{
    ...
    AnyPromise * (^failWithUnexpectedError)(void) = ^{
        NSError *error = [NSError errorWithDomain:OWSBackupErrorDomain
                                             code:1
                                         userInfo:@{
                                             NSLocalizedDescriptionKey : NSLocalizedString(@"XXGJUSTHHANQB20",
