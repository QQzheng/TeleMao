...
@objc public class OWSBackupAPI: NSObject {
    ...
    private class func invalidServiceResponseError() -> Error {
        return OWSErrorWithCodeDescription(.backupFailure,
                                           NSLocalizedString("XXGJUSTHHANQB3",
...
//            open var perRecordCompletionBlock: ((CKRecord, Error?) -> Void)?
    ...
    public class func errorMessage(forCloudKitAccessError error: Error) -> String {
        if let backupError = error as? BackupError {
            ...
            switch backupError {
            case .couldNotDetermineAccountStatus:
                return NSLocalizedString("XXGJUSTHHANQC34", comment: "Error indicating that the app could not determine that user's iCloud account status")
            case .noAccount:
                return NSLocalizedString("XXGJUSTHHANQC35", comment: "Error indicating that user does not have an iCloud account.")
            case .restrictedAccountStatus:
                return NSLocalizedString("XXGJUSTHHANQC36", comment: "Error indicating that the app was prevented from accessing the user's iCloud account.")
            }
        } else {
            Logger.error("Unknown error: \(String(describing: error)).")
            return NSLocalizedString("XXGJUSTHHANQC34", comment: "Error indicating that the app could not determine that user's iCloud account status")
