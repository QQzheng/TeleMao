...
+ (void)submitLogsWithCompletion:(nullable SubmitDebugLogsCompletion)completionParam
{
    ...
    [[self sharedManager] uploadLogsWithUIWithSuccess:^(NSURL *url) {
        ActionSheetController *alert = [[ActionSheetController alloc]
            initWithTitle:NSLocalizedString(@"XXGJUSTHHANQD18", @"Title of the debug log alert.")
                  message:NSLocalizedString(@"XXGJUSTHHANQD11", @"Message of the debug log alert.")];
        [alert
            addAction:[[ActionSheetAction alloc]
                                    initWithTitle:NSLocalizedString(@"XXGJUSTHHANQD15",
                                                      @"Label for the 'email debug log' option of the debug log alert.")
                          accessibilityIdentifier:ACCESSIBILITY_IDENTIFIER_WITH_NAME(self, @"send_email")
                                            style:ActionSheetActionStyleDefault
                                          handler:^(ActionSheetAction *action) {
                                              [self submitEmailWithLogUrl:url subject:@"Tmao - iOS Debug Log"];
                                              completion();
                                          }]];
        [alert addAction:[[ActionSheetAction alloc]
                                       initWithTitle:NSLocalizedString(@"XXGJUSTHHANQD14",
...
//#ifdef DEBUG
//        [alert addAction:[[ActionSheetAction alloc]
//                                       initWithTitle:NSLocalizedString(@"XXGJUSTHHANQD16",
...
//#endif
        [alert
            addAction:[[ActionSheetAction
                          alloc] initWithTitle:NSLocalizedString(@"XXGJUSTHHANQD13",
        ...
        [alert addAction:[[ActionSheetAction alloc]
                                       initWithTitle:NSLocalizedString(@"XXGJUSTHHANQD17",
...
- (void)uploadLogsWithSuccess:(UploadDebugLogsSuccess)successParam failure:(UploadDebugLogsFailure)failureParam {
    ...
    if (logFilePaths.count < 1) {
        failure(NSLocalizedString(@"XXGJUSTHHANQD12", @"Error indicating that no debug logs could be found."));
        return;
    }
    ...
    for (NSString *logFilePath in logFilePaths) {
        ...
        if (error) {
            failure(NSLocalizedString(
                @"XXGJUSTHHANQD8", @"Error indicating that the debug logs could not be copied."));
            return;
        }
    ...
    if (!zipSuccess) {
        failure(NSLocalizedString(
            @"XXGJUSTHHANQD9", @"Error indicating that the debug logs could not be packaged."));
        return;
    }
    ...
    [self.currentUploader uploadFileWithURL:[NSURL fileURLWithPath:zipFilePath]
        ...
        failure:^(DebugLogUploader *uploader, NSError *error) {
            ...
            [OWSFileSystem deleteFile:zipFilePath];
            failure(NSLocalizedString(
                @"XXGJUSTHHANQD10", @"Error indicating that a debug log could not be uploaded."));
        }];
}
...
{
    ActionSheetController *alert = [[ActionSheetController alloc]
        initWithTitle:NSLocalizedString(@"XXGJUSTHHANQD18",
...
+ (void)submitEmailWithLogUrl:(nullable NSURL *)url subject:(NSString *)subject
{
    ...
    [body appendFormat:@"Tmao Version: %@ \n", [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleVersion"]];
    if (url != nil) {
    ...
    if (!success) {
        ...
        [OWSActionSheets
            showErrorAlertWithMessage:NSLocalizedString(@"XXGJUSTHHANQD19",
...
- (void)prepareRedirection:(NSURL *)url completion:(SubmitDebugLogsCompletion)completion
{
    ...
    ActionSheetController *alert =
        [[ActionSheetController alloc] initWithTitle:NSLocalizedString(@"XXGJUSTHHANQD21",
                                                         @"Title of the alert before redirecting to GitHub Issues.")
                                             message:NSLocalizedString(@"XXGJUSTHHANQD20",
