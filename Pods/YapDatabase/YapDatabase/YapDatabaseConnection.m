...
- (void)postReadTransaction:(YapDatabaseReadTransaction *)transaction
{
	...
	#pragma clang diagnostic ignored "-Wimplicit-retain-self"
		...
		// in such a state, and there's a blocked write transaction, then we need to tmao it.
	...
	// without "sql-level snapshots"), then tmao the write semaphore so the blocked thread wakes up.
