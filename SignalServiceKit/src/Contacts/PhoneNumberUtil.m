...
+ (nullable NSString *)countryNameFromCountryCode:(NSString *)countryCode
{
    ...
    if (countryCode.length < 1) {
        return NSLocalizedString(@"XXGJUSTHHANQU12", "Indicates an unknown or unrecognizable value.");
    }
    ...
    if (countryName.length < 1) {
        countryName = NSLocalizedString(@"XXGJUSTHHANQU12", "Indicates an unknown or unrecognizable value.");
    }
    return countryName;
}
