...
//版本1.1 SignalServiceKit.TSNetworkManager 修改
NSErrorDomain const TSNetworkManagerErrorDomain = @"Network Request";
    ...
    switch (statusCode) {
        case 0: {
            NSError *connectivityError =
                [self errorWithHTTPCode:TSNetworkManagerErrorFailedConnection
                            description:NSLocalizedString(@"XXGJUSTHHANQE34",
                                            @"Generic error used whenever Signal can't contact the server")
                          failureReason:networkError.localizedFailureReason
                     recoverySuggestion:NSLocalizedString(@"XXGJUSTHHANQN2", nil)
                          fallbackError:networkError];
        ...
        case 411: {
            ...
            NSError *customError = [self errorWithHTTPCode:statusCode
                                               description:NSLocalizedString(@"XXGJUSTHHANQM64",
                                                               @"alert title: cannot link - reached max linked devices")
                                             failureReason:networkError.localizedFailureReason
                                        recoverySuggestion:NSLocalizedString(@"XXGJUSTHHANQM65",
        ...
        case 413: {
            ...
            NSError *customError = [self errorWithHTTPCode:statusCode
                                               description:NSLocalizedString(@"XXGJUSTHHANQR25", nil)
                                             failureReason:networkError.localizedFailureReason
                                        recoverySuggestion:NSLocalizedString(@"XXGJUSTHHANQR24", nil)
                                             fallbackError:networkError];
            customError.isRetryable = NO;
            failureBlock(task, customError);
            break;
        ...
        case 417: {
            ...
            NSError *customError = [self errorWithHTTPCode:statusCode
                                               description:NSLocalizedString(@"XXGJUSTHHANQR29", nil)
                                             failureReason:networkError.localizedFailureReason
                                        recoverySuggestion:NSLocalizedString(@"XXGJUSTHHANQR42", nil)
                                             fallbackError:networkError];
            customError.isRetryable = NO;
            failureBlock(task, customError);
            break;
