...
public class VideoEditorView: UIView {
    ...
    public func configureSubviews() {
        ...
        playButton.accessibilityLabel = NSLocalizedString("XXGJUSTHHANQP65", comment: "Accessibility label for button to start media playback")
        ...
        viewController.ows_askForMediaLibraryPermissions { isGranted in
            ...
            ModalActivityIndicatorViewController.present(fromViewController: viewController, canCancel: false) { modalVC in
                DispatchQueue.global().async(.promise) {
                    ...
                    modalVC.dismiss {
                        OWSActionSheets.showErrorAlert(message: NSLocalizedString("XXGJUSTHHANQE30", comment: "Error indicating that 'save video' failed."))
                    }
                }.retainUntilComplete()
            }
        }
