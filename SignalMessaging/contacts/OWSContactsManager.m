...
+ (NSString *)accountLabelForContact:(Contact *)contact address:(SignalServiceAddress *)address
{
    ...
    if (addressesWithTheSameName.count > 1) {
        ...
        addressLabel =
            [NSString stringWithFormat:NSLocalizedString(@"XXGJUSTHHANQP12",
...
- (NSString *)unknownContactName
{
    return NSLocalizedString(
        @"XXGJUSTHHANQU3", @"Displayed if for some reason we can't determine a contacts phone number *or* name");
}
                                          ...
                                          secondaryAttributes:(NSDictionary *)secondaryAttributes
{
    ...
    if (savedContactName.length > 0) { // 通讯录名称
...
//                                                                                 attributes:secondaryAttributes]];
//        return [result copy];
        return [[NSAttributedString alloc] initWithString:profileName attributes:primaryAttributes];
    }
