...
- (void)ows_askForCameraPermissions:(void (^)(BOOL granted))callbackParam
{
    ...
    if (status == AVAuthorizationStatusDenied) {
        ActionSheetController *alert = [[ActionSheetController alloc]
            initWithTitle:NSLocalizedString(@"XXGJUSTHHANQM59", @"Alert title")
                  message:NSLocalizedString(@"XXGJUSTHHANQM58", @"Alert body")];
...
- (void)ows_askForMediaLibraryPermissions:(void (^)(BOOL granted))callbackParam
{
    ...
    void (^presentSettingsDialog)(void) = ^(void) {
        DispatchMainThreadSafe(^{
            ActionSheetController *alert = [[ActionSheetController alloc]
                initWithTitle:NSLocalizedString(@"XXGJUSTHHANQM63",
                                  @"Alert title when user has previously denied media library access")
                      message:NSLocalizedString(@"XXGJUSTHHANQM62",
...
- (void)ows_showNoMicrophonePermissionActionSheet
{
    DispatchMainThreadSafe(^{
        ActionSheetController *alert = [[ActionSheetController alloc]
            initWithTitle:NSLocalizedString(@"XXGJUSTHHANQB50",
                              @"Alert title when calling and permissions for microphone are missing")
                  message:NSLocalizedString(@"XXGJUSTHHANQB49",
