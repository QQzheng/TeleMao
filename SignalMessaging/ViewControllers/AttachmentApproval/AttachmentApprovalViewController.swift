...
public class AttachmentApprovalViewController: UIPageViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate {
    ...
    public func updateNavigationBar() {
        ...
        guard !isEditingCaptions else {
            ...
            self.navigationItem.leftBarButtonItem = UIBarButtonItem(title: NSLocalizedString("XXGJUSTHHANQA41", comment: "Title for 'caption' mode of the attachment approval view."), style: .plain, target: nil, action: nil)
    ...
    public func didTapSave() {
            let errorText = NSLocalizedString("XXGJUSTHHANQA42",
                ...
                self.ows_askForMediaLibraryPermissions { isGranted in
                    ...
                    PHPhotoLibrary.shared().performChanges({
                        ...
                        DispatchQueue.main.async {
                            if didSucceed {
                                let toastController = ToastController(text: NSLocalizedString("XXGJUSTHHANQA45",
        ...
        ModalActivityIndicatorViewController.present(fromViewController: self, canCancel: false) { modalVC in
            self.outputAttachmentsPromise()
                .done { attachments in
                    ...
                    owsFailDebug("Error: \(error)")
                    modalVC.dismiss {
                        OWSActionSheets.showErrorAlert(message: NSLocalizedString("XXGJUSTHHANQA75", comment: "Error that outgoing attachments could not be exported."))
                    }
                }.retainUntilComplete()
        }
    }
