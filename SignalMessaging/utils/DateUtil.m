...
+ (NSString *)formatPastTimestampRelativeToNow:(uint64_t)pastTimestamp
{
    ...
    if (isFutureTimestamp || [self dateIsToday:pastDate]) {
        dateString = NSLocalizedString(@"XXGJUSTHHANQD6", @"The current day.");
    } else if ([self dateIsYesterday:pastDate]) {
        dateString = NSLocalizedString(@"XXGJUSTHHANQD7", @"The day before today.");
    } else {
...
+ (NSString *)formatMessageTimestamp:(uint64_t)timestamp
{
    ...
    if (hoursDiff < 1 && minutesDiff < 1) {
        return NSLocalizedString(@"XXGJUSTHHANQD5", @"The present; the current time.");
    }

    if (hoursDiff < 1) {
        NSString *minutesString = [OWSFormat formatInt:(int)minutesDiff];
        return [NSString stringWithFormat:NSLocalizedString(@"XXGJUSTHHANQD4",
    ...
    if (daysDiff >= 7) {
        ...
        return [NSString stringWithFormat:NSLocalizedString(@"XXGJUSTHHANQD3",
                                              @"Format string for a relative time, expressed as a certain number of "
...
{
    return NSLocalizedString(@"XXGJUSTHHANQD5", @"The present; the current time.").localizedUppercaseString;
}
